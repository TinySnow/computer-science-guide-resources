#!/usr/bin/env python3
# =============================================================================
# 计算机科学极简入门指南 - 封面图批量生成工具
# =============================================================================
#
# 【功能】
#   读取 XMind 思维导图或纯文本列表，为每个主题生成一张标题图片。
#   图片样式：纯白背景，第一行“计算机科学极简入门指南”，第二行为具体主题，
#   画布固定为 780 × 330px，与现有封面保持一致。
#   主题清洗规则（用于标题展示）：
#     - 删除（...）与 (...) 括号段（非贪婪）
#     - 清理多余空白
#   树形落盘规则（XMind 模式）：
...
#
#   ① 从 XMind 文件生成全部画布：
#         python3 generate_covers.py --xmind guide.xmind
#      输出结构：
#         output/
#         ├── 画布名/
#         │   └── 根结点/
...
#         python3 generate_covers.py --xmind guide.xmind --sheet 第一章
#      仅处理指定画布，其他画布不受影响。
#      画布名称需与 XMind 中的标签页名称完全一致。
#      也可进一步局部更新：
#         python3 generate_covers.py --xmind guide.xmind --sheet 第一章 --path 计算机的构成
#      仅处理指定路径前缀下的主题。
#
#   ③ 列出 XMind 中所有画布名称（不生成图片，用于确认 --sheet 的名称）：
#         python3 generate_covers.py --xmind guide.xmind --list-sheets
#
#   ④ 生成单张图片（临时测试用）：
#         python3 generate_covers.py --topic "总览"
#      图片直接输出到 OUTPUT_DIR 根目录，不建子文件夹。
#      若希望指定输出目录路径（无需手打文件名）：
#         python3 generate_covers.py --topic-path "第一章/计算机的构成"
#      未传 --topic 时，会自动取最后一段作为标题和文件名；
#      只传 --topic-path 时，最后一段为文件名，其余段为目录。
#
#   ⑤ 从纯文本文件批量生成（不使用 XMind 时）：
#         python3 generate_covers.py --list topics.txt
#      topics.txt 格式：每行一个主题，# 开头为注释，空行忽略。
#      若文件不存在，脚本会自动创建一个示例文件。
#
#   ⑥ 从封面清单批量生成（适合本指南的分目录封面）：
#         python3 generate_covers.py --manifest covers.tsv
#      每行格式：目录<TAB>展示标题<TAB>文件名；目录可留空（总览）。
#
# 【配置】
...
FONT_PATH  = PROJECT_ROOT / "assets" / "仓耳与墨W02.ttf"
OUTPUT_DIR = PROJECT_ROOT / "computer-science-guide" / "cover"
FONT_SIZE  = 45                    # 字号（px），与现有 cover 的文字大小一致
IMG_WIDTH  = 780                   # 图片宽度（px），固定不变
IMG_HEIGHT = 330                   # 图片高度（px），固定不变
PADDING_X  = 60                    # 左右安全边距（px）
LINE_GAP   = 12                    # 两行文字之间的间距（px）
VERTICAL_OFFSET = -8               # 视觉校正：与现有 cover 的文字垂直位置一致
TEXT_COLOR = "#3d3d3d"             # 文字颜色
BG_COLOR   = "#ffffff"             # 背景颜色
TITLE_LINE = "计算机科学极简入门指南"  # 固定第一行文字
# ─────────────────────────────────────────────────────────────
...
def load_font(size: int) -> ImageFont.FreeTypeFont:
    ...
    if not path.exists():
        ...
        print("        请将字体文件放到项目 assets 目录，或修改脚本顶部 FONT_PATH 变量。")
        sys.exit(1)
...
def make_image(topic: str, font: ImageFont.FreeTypeFont) -> Image.Image:
    ...
    原理：
      ...
      2. 图片尺寸固定为 IMG_WIDTH × IMG_HEIGHT；两行文字均在画布中居中。
    ...
    def measure(text, text_font):
        # textbbox 返回 (left, top, right, bottom)，差值即为宽高
        bbox = draw.textbbox((0, 0), text, font=text_font)
        return bbox[2] - bbox[0], bbox[3] - bbox[1]

    w1, h1 = measure(TITLE_LINE, font)
    topic_font = font
    w2, h2 = measure(topic, topic_font)
    while w2 + PADDING_X * 2 > IMG_WIDTH and topic_font.size > 28:
        topic_font = load_font(topic_font.size - 1)
        w2, h2 = measure(topic, topic_font)

    # ── 步骤 2：计算图片尺寸和文字起始坐标 ──
    img_width    = IMG_WIDTH
    if max(w1, w2) + PADDING_X * 2 > img_width:
        raise ValueError(
            f"标题过长，无法放入 {IMG_WIDTH}px 宽的封面：{topic}。"
            "请缩短标题，或调小 FONT_SIZE。"
        )
    total_text_h = h1 + LINE_GAP + h2
    y_start      = (IMG_HEIGHT - total_text_h) // 2 + VERTICAL_OFFSET
    ...
    draw.text(((img_width - w2) // 2, y_start + h1 + LINE_GAP),   topic,      font=topic_font, fill=TEXT_COLOR)
...
def strip_parenthesized(text: str) -> str:
    ...
    设计意图：
      ...
      - 封面文件名由调用方独立传入，因此保留冒号后的语义内容

    例：
      （注意）结点（补充）  ->  结点
      node(foo)bar(baz)     ->  nodebar
      第一篇 基础与大脑：你是谁？ -> 第一篇 基础与大脑：你是谁？
    """
...
def read_manifest_file(path: str) -> list[tuple[list[str], str, str]]:
    """读取分目录封面清单：目录<TAB>展示标题<TAB>文件名。"""
    items = []
    with open(path, encoding="utf-8") as f:
        for line_no, line in enumerate(f, 1):
            ...
            parts = raw.split("\t")
            if len(parts) != 3:
                raise ValueError(f"{path}:{line_no} 应包含 3 列（目录、展示标题、文件名）。")
            folder, topic, filename = (part.strip() for part in parts)
            if not topic or not filename:
                raise ValueError(f"{path}:{line_no} 的展示标题和文件名不能为空。")
            items.append((parse_path_expr(folder), strip_parenthesized(topic) or topic, filename))
    return items
...
def main():
    ...
    parser.add_argument("--xmind",       metavar="FILE",
                        help="指定 XMind 文件路径；未提供时使用单图或文本列表模式")
    ...
    parser.add_argument("--manifest",    metavar="FILE",
                        help="从分目录封面清单批量生成：目录<TAB>展示标题<TAB>文件名")

    args = parser.parse_args()
    # 模式优先级（从高到低）：
    #   --list-sheets > XMind 批量模式 > 清单模式 > 单图模式 > 文本列表模式
    ...
    # ── 模式三：从分目录清单批量生成 ─────────────────────────
    if args.manifest:
        items = read_manifest_file(args.manifest)
        print(f"从 {args.manifest} 读取到 {len(items)} 个封面\n")
        for subfolders, topic, filename_topic in items:
            img = make_image(topic, font)
            save_image(img, topic, subfolders=subfolders, filename_topic=filename_topic)
        print(f"\n全部完成，共生成 {len(items)} 张图片。")
        return
